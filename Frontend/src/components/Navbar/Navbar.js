...
export default function Navbar() {
  ...
  const isSM = user?.userFound?.isSM;
  const isPM = user?.userFound?.isPM;

  
  //logout handler
  const logoutHandler = () => {
    dispatch(logoutAction());
    // Reload or redirect based on user role
    if (user?.userFound?.isPM || user?.userFound?.isSM) {
                ...
                <div className="flex h-16 items-center justify-between">
                  ...
                  <div className="flex flex-1 items-center justify-end">
                    ...
                    <div className="flex items-center lg:ml-8">
                      ...
                      {/* login Wishlist */}
                      {isLoggedIn && (
                      <div className="flow-root space-y-6 border-gray-200 py-6 px-4">
                        <Link
                          to="/wishlist"
