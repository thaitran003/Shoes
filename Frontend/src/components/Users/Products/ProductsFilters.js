...
const sortOptions = [
  ...
  { name: "Price: Low to High", href: "priceasc", current: false },
  { name: "Price: High to Low", href: "pricedes", current: false },
];
...
const sizeCategories = [38, 39, 40, 41, 42, 43, 44, 45];
...
export default function ProductsFilters() {
  ...
  const [sortOptions, setSortOptions] = useState([
    { name: "Most Popular", href: "popularity", current: true },
    { name: "Price: Low to High", href: "priceasc", current: false },
    { name: "Price: High to Low", href: "pricedes", current: false },
  ]);
  ...
  const resetFilters = () => {
    ...
    // SAYFA YENILEYINCE BILE SORT OPTIONS SIFIRLANIYOR, DUZELTMEMIZ LAZIM 
  };
  ...
  if (searchQuery) {
    productUrl = `${productUrl}&search=${searchQuery}`;
  }
  if (brand) {
  ...
  }, [dispatch, category, size, brand, price, color, sort, searchQuery]);
  //get store data
  const {
    products: { products },
    loading,
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div>
            ...
            <div>
                ...
                />
                <i className="fas fa-search search-icon" style={{marginLeft: "5px"}}/>
            </div>
                        ...
                        <Menu.Item key={option.name}>
                          {({ active }) => (
                              ...
                              onClick={() => {
                                ...
                                updatedSortOptions.forEach((sortOption) => {
                                  if (option.name === sortOption.name) {
