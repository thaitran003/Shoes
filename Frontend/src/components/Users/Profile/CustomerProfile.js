...
import { createRRaction } from "../../../redux/slices/refundRequests/refundRequests";
...
export default function CustomerProfile() {
  ...
  const { isAdded, error: RequestError } = useSelector((state) => state?.requests);
  const refundrequest = (order, prods, msg) => {
    //itemIds, refundReasons
    dispatch(createRRaction({ orderID: order, itemIds: prods, refundReasons: msg }));
  ...
  const cancelOrderHandler = (id) => {
    ...
    // SuccessMsg({"Order canceled successfully"});
    window.location.reload();
  }
  
  const [refundMessages, setRefundMessages] = useState({});

  const handleRefundMessageChange = (productId, value) => {
    setRefundMessages((prevState) => ({
      ...prevState,
      [productId]: value,
    }));
  };
    ...
    <>
      ...
      {loading ? (
        ...
        orders?.map((order) => {
          return (
            <>
              {isAdded && <SuccessMsg message="Refund Request successfully made" />}
              {RequestError && <ErrorMsg message={RequestError?.message} />}
              <div className="bg-gray-50">
                ...
                >
                  ...
                  <div className="flex">
                    <input
                      type="text"
                      id={`refundMessageInput_${product.id}`}
                      placeholder="Enter refund message"
                      className="mr-2"
                      value={refundMessages[product.id] || ''}
                      onChange={(e) => handleRefundMessageChange(product.id, e.target.value)}
                    />
                    <button
                      onClick={() => refundrequest(order._id, [product._id], refundMessages[product.id])}
                    ...
                    <div className="mt-8 border-t border-gray-200 border-t border-b border-gray-200 bg-white shadow-sm sm:rounded-lg sm:border">
                    {/* shipping address */}
